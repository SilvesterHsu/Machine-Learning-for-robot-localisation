...
BAG_PATH="INSERT BAG LOCATION HERE"
MAP_PATH="INSERT MAP LOCATION HERE, IT CAN BE ANY MAP, DOESNT MATTER"
HEATMAP_PATH="INSERT ANY FILTER HERE>heatmap.hmp"
...
#MAP_PATH=$MAP_LOCATION/michigan/2012_05_11_map/offmichigan_gt=1_submap=1_sizexy=200_Z=17_intrchR=60_compR=10_res=1_maxSensd=130_keyF=1_d=0.4_deg=150_alpha=0_dl=0_xyzir=0_mpsu=0_mnpfg=3kmnp1.map
...
#rosrun self_localization localization_offline --nn-estimates --convergance-rate 0.5 --bag-start-index 0 --random-initial-pose --visualize --save-heatmap --load-heatmap --heatmap-path $HEATMAP_PATH --initial_noise 2 2 2 0 0 0    --init-ex 3.1415 --score-cell-weight 0.1 --ms-delay 0  --disable-unwarp  --n-particles 1000 --forceSIR  --map-switching-method closest_observation --min-range 2 --keyframe-min-distance 0 --keyframe-min-rot-deg 100 --key-frame-update --tf-base-link /robot_odom_link --tf-gt-link /state_base_link --localisation-algorithm-name submap_mcl --skip-frame 1 --base-name mcl-ndt --bag-file-path $BAG_PATH --map-file-path $MAP_PATH  --data-set michigan-noisy --z-filter-height -1000000  --velodyne-config-file "$(rospack find graph_map)/config/velo32.yaml" --lidar-topic /velodyne_points resolution-local-factor 2.0   --save-results --step-control
