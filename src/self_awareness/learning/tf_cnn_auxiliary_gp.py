    ...
    def __init__(self, args, is_training=True):
        ...
        if is_training:
            with tf.variable_scope('adam'):
                cnn_tvars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='context_stack_global')
                regressor_vars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='regressor_global')
                ...
                '''Rotation loss'''
                rot_loss = 1. - tf.reduce_mean(tf.square(tf.reduce_sum(tf.multiply(rot_pred, rot_target), axis=-1)))
                self.rot_loss = tf.cast(rot_loss, dtype=float_type)
                ...
                regressor_optimizer = tf.train.AdamOptimizer(self.lr * 0.01)
                regressor_grad_vars = regressor_optimizer.compute_gradients(loss=self.total_loss, var_list=regressor_vars)

                cnn_optimizer = tf.train.AdamOptimizer(self.lr * 0.001)
                ...
                self.train_op = tf.group(gp_optimizer.apply_gradients(gp_grad_vars, global_step=self.global_step),
                                         cnn_optimizer.apply_gradients(cnn_grad_vars), regressor_optimizer.apply_gradients(regressor_grad_vars))
            ...
            # pred_feat = tf.concat([trans_feat, rot_feat], axis=1)

            c_mean, c_var = self.gp_model._build_predict(tf.cast(trans_feat, dtype=float_type), full_cov=False, full_output_cov=False)
            y_mean, y_var = self.gp_model.likelihood.predict_mean_and_var(c_mean, c_var)

            trans_pred = tf.cast(y_mean, dtype=tf.float32)
            _, rot_pred = tf.split(global_output, [3, 4], axis=1)

            dist = tfd.Normal(loc=tf.reshape(c_mean, [1, 3]), scale=tf.math.multiply(c_var, tf.reshape(tf.cast(self.norm_std, dtype=float_type), [1, 3])))
            samples = tf.cast(tf.reshape(dist.sample([100]), [100, 3]), dtype=tf.float32)

            self.distribution_mean = tf.cast(c_mean, dtype=tf.float32)
            self.distribution_cov = tf.math.multiply(tf.cast(c_var, dtype=tf.float32),  tf.reshape(self.norm_std, [1, 3]))
