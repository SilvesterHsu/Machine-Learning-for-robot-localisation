...
class Model():
    ...
    def __init__(self, args, is_training=True):
        ...
        f_X = tf.cast(pose_feat, dtype=float_type)
        Y = tf.cast(target_normed, dtype=float_type)
        ...
        with tf.variable_scope('gp'):
            ...
            # kernel = mk.SeparateMixedMok([gpflow.kernels.RBF(args.feat_dim, ARD=False, name="rbf_ard"), gpflow.kernels.Linear(args.feat_dim, ARD=False, name="linear_ard")], np.zeros((args.output_dim, 2)))
            q_mu = np.zeros((args.batch_size, args.output_dim)).reshape(args.batch_size * args.output_dim, 1)
            q_sqrt = np.eye(args.batch_size * args.output_dim).reshape(args.batch_size, args.output_dim, args.batch_size, args.output_dim)
            # feature = gpflow.features.InducingPoints(np.zeros((args.batch_size, 128)))

            # self.gp_model = gpflow.models.SVGP(X=f_X, Y=Y, kern=kernel,
            #                                    likelihood=gpflow.likelihoods.Gaussian(name="lik"),
            #                                    Z=np.zeros((args.batch_size, args.feat_dim)), q_mu=q_mu, q_sqrt=q_sqrt,
            #                                    q_diag=False, whiten=False, name="svgp")
            self.gp_model = gpflow.models.SVGP(X=f_X, Y=Y, kern=kernel, likelihood=gpflow.likelihoods.Gaussian(name="lik"),
        ...
        if is_training:
            with tf.variable_scope('adam'):
                ...
                self.total_loss = -self.gp_model.likelihood_tensor / args.batch_size
                self.trans_loss = self.total_loss
                self.rot_loss = self.total_loss
            ...
            _, pred_trans_feat, pred_rot_feat = network.build(self.pred_data, is_training)
            pred_feat = tf.concat([pred_trans_feat, pred_rot_feat], axis=1)

            c_mean, c_var = self.gp_model._build_predict(tf.cast(pred_feat, dtype=float_type), full_cov=False, full_output_cov=False)
            t_mean, r_mean = tf.split(c_mean, [3, 4], axis=1)
            y_mean, y_var = self.gp_model.likelihood.predict_mean_and_var(c_mean, c_var)

            pred_output = tf.cast(y_mean, dtype=tf.float32)
            trans_pred, rot_pred = tf.split(pred_output, [3, 4], axis=1)

            dist = tfd.Normal(loc=tf.reshape(t_mean, [1,3], dtype=tf.float32), scale=tf.reshape(c_var*1000., [1, 3], dtype=tf.float32))
