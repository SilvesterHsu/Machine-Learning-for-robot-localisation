...
def densenet(inputs,
  ...
  with tf.variable_scope(scope, 'densenetxxx', [inputs, num_classes],
    ...
    with slim.arg_scope([slim.batch_norm, slim.dropout],
      ...
      # initial convolution
      net = slim.conv2d(net, num_filters, 7, stride=4, scope='conv1')
...
def densenet121(inputs, num_classes=1000, data_format='NHWC', is_training=True, is_global_pool=True, reuse=None, scope='densenet121'):
  return densenet(inputs,
                  ...
                  is_training=is_training,
                  global_pool=is_global_pool,
                  reuse=reuse,
                  scope=scope)
densenet121.default_image_size = 224


def densenet161(inputs, num_classes=1000, data_format='NHWC', is_training=True, is_global_pool=True, reuse=None, scope='densenet161'):
  return densenet(inputs,
                  num_classes=num_classes, 
                  reduction=0.5,
                  growth_rate=48,
...
def densenet169(inputs, num_classes=1000, data_format='NHWC', is_training=True, is_global_pool=True, reuse=None, scope='densenet169'):
  return densenet(inputs,
                  num_classes=num_classes, 
                  reduction=0.5,
                  growth_rate=32,
